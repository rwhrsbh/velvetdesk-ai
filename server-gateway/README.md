...
Адрес шлюза вшивается в приложение на сборке — IP годится так же, как имя:

```sh
VD_CLOUD_BASE_URL=https://203-0-113-10.sslip.io/v1 VD_LICENSE_PUBLIC_KEY='вывод keygen' npm run desktop:build
```

В CI это секреты `VD_CLOUD_BASE_URL` и `VD_LICENSE_PUBLIC_KEY`; сборка без
них — бесплатная для всех и без облачного провайдера.

### TLS без покупки домена

`http://IP` означает, что ключ лицензии и вся переписка идут по интернету
открытым текстом. Домен для этого покупать не нужно:
`203-0-113-10.sslip.io` (дефисы вместо точек) бесплатно резолвится в
`203.0.113.10`, и на это имя Let's Encrypt выдаёт обычный сертификат.

```sh
cp server-gateway/Caddyfile.example Caddyfile   # вписать свой адрес и почту
docker compose --profile tls up -d
```

Caddy сам получит и продлит сертификат, WebSocket синхронизации пройдёт
насквозь. Голый IP тоже можно (`tls internal`), но сертификат будет
самоподписанный, и приложение его не примет — это для стенда.

Если всё же нужен чистый HTTP, помните: Android по умолчанию его запрещает;
сборка с `http://` в `VD_CLOUD_BASE_URL` включает исключение сама и пишет
предупреждение в лог сборки.
