...
| --- | --- | --- |
| **На устройстве** | скачать модель один раз (39–240 МБ) | никуда, всё локально |
| Gemini | ключ | в Google |
| OpenAI-совместимый | ключ (Groq, OpenAI, Together, Mistral) или локальный сервер | в выбранный сервис |

Локальные модели — квантованный Whisper из репозиториев `onnx-community`, все мультиязычные (ru/uk/en):

| Модель | Размер | Когда брать |
| --- | --- | --- |
| tiny | ~41 МБ | короткие заметки, самая быстрая |
| base | ~76 МБ | баланс |
| small | ~240 МБ | лучшее качество для русского и украинского |
