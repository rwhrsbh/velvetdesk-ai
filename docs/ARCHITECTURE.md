...
## 0. Раскладка репозитория

```
crates/vd-llm/      вызовы провайдеров, пул ключей, каталог моделей
crates/vd-license/  подпись и проверка лицензий (Ed25519)
src-tauri/          десктоп
server-gateway/     шлюз на VPS: лицензии, кредиты, релей синхронизации
src/                фронтенд
```

`vd-llm` и `vd-license` общие: клиент и шлюз обязаны одинаково ротировать
ключи и одинаково читать лицензию, а две копии такого кода расходятся за
месяц. Подробности коммерческой части — `docs/ROADMAP_AGENT.md`.
